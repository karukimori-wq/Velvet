...
type Row = { id:string; workspace_id:string; user_id:string; customer_id:string; text:string; status:"open"|"done"|"cancelled"; created_at:string; due_at?:string|null; completed_at:string|null; action_type?:string|null; topic_id?:string|null; timing?:string|null; priority?:string|null; source_capture_id?:string|null; source_topic_id?:string|null };
const map = (r: Row): ProfessionalNextAction => ({ id:r.id, workspaceId:r.workspace_id, userId:r.user_id, customerId:r.customer_id, text:r.text, status:r.status, createdAt:new Date(r.created_at).toISOString(), dueAt:r.due_at?new Date(r.due_at).toISOString():undefined, completedAt:r.completed_at?new Date(r.completed_at).toISOString():undefined, actionType:r.action_type??undefined, topicId:r.topic_id??undefined, timing:r.timing==="next_visit"?"next_visit":r.timing==="date"?"date":undefined, priority:r.priority==="high"?"high":r.priority==="low"?"low":r.priority==="normal"?"normal":undefined, sourceCaptureId:r.source_capture_id??undefined, sourceTopicId:r.source_topic_id??undefined });
...
export async function countDueNextActions(workspaceId: string, userId: string, through: Date) {
  const throughIso = through.toISOString();
  if (getStorageMode() === "d1") {
    const db = await getD1Database(); if (!db) return 0;
    const row = await db.prepare("select count(*) as count from velvet_professional_next_actions where workspace_id=? and user_id=? and status='open' and due_at is not null and due_at<=?").bind(workspaceId,userId,throughIso).first<{count:number}>();
    return Number(row?.count ?? 0);
  }
  if (getStorageMode() !== "postgres") return rows.filter(r => r.workspaceId === workspaceId && r.userId === userId && r.status === "open" && r.dueAt && r.dueAt <= throughIso).length;
  const result = await dbQuery<{count:string}>(`select count(*)::text as count from velvet_professional_next_actions where workspace_id=$1 and user_id=$2 and status='open' and due_at is not null and due_at<=$3`, [workspaceId,userId,throughIso]);
  return Number(result.rows[0]?.count ?? 0);
}

export async function createNextAction(workspaceId:string,userId:string,customerId:string,text:string,dueAt?:string,metadata:{actionType?:string;topicId?:string;timing?:"date"|"next_visit";priority?:"high"|"normal"|"low";sourceCaptureId?:string;sourceTopicId?:string}={}){const normalizedDueAt=normalizeDueAt(dueAt);const item:ProfessionalNextAction={id:getStorageMode()==="d1"?makeD1Id("next"):makeId(),workspaceId,userId,customerId,text:text.trim(),status:"open",createdAt:new Date().toISOString(),dueAt:normalizedDueAt,...metadata};if(getStorageMode()==="d1"){const db=await getD1Database();if(!db)throw new Error("D1_NOT_CONFIGURED");await db.prepare("insert into velvet_professional_next_actions(id,workspace_id,user_id,customer_id,text,status,created_at,due_at,action_type,topic_id,timing,priority,source_capture_id,source_topic_id) values(?,?,?,?,?,'open',?,?,?,?,?,?,?,?)").bind(item.id,workspaceId,userId,customerId,item.text,item.createdAt,item.dueAt??null,item.actionType??null,item.topicId??null,item.timing??null,item.priority??null,item.sourceCaptureId??null,item.sourceTopicId??null).run()}else if(getStorageMode()!=="postgres")rows.unshift(item);else await dbQuery(`insert into velvet_professional_next_actions(id,workspace_id,user_id,customer_id,text,status,created_at,due_at,action_type,topic_id,timing,priority,source_capture_id,source_topic_id) values($1,$2,$3,$4,$5,'open',$6,$7,$8,$9,$10,$11,$12,$13)`,[item.id,workspaceId,userId,customerId,item.text,item.createdAt,item.dueAt??null,item.actionType??null,item.topicId??null,item.timing??null,item.priority??null,item.sourceCaptureId??null,item.sourceTopicId??null]);await addProfessionalTimelineItem({workspaceId,userId,customerId,eventType:"next_action",title:"次回アクション",body:[item.text,item.dueAt?`期限 ${item.dueAt.slice(0,10)}`:item.timing==="next_visit"?"次回来店時":undefined,item.priority?`優先度 ${item.priority}`:undefined].filter(Boolean).join(" · "),sourceRef:item.id});return item}
...
export async function cancelNextAction(workspaceId:string,userId:string,customerId:string,id:string){if(getStorageMode()==="d1"){const db=await getD1Database();if(!db)return false;const existing=await db.prepare("select id from velvet_professional_next_actions where id=? and workspace_id=? and user_id=? and customer_id=?").bind(id,workspaceId,userId,customerId).first();if(!existing)return false;await db.prepare("update velvet_professional_next_actions set status='cancelled' where id=? and workspace_id=? and user_id=? and customer_id=?").bind(id,workspaceId,userId,customerId).run()}else if(getStorageMode()!=="postgres"){const item=rows.find(r=>r.id===id&&r.workspaceId===workspaceId&&r.userId===userId&&r.customerId===customerId);if(!item)return false;item.status="cancelled"}else{const result=await dbQuery(`update velvet_professional_next_actions set status='cancelled' where id=$1 and workspace_id=$2 and user_id=$3 and customer_id=$4`,[id,workspaceId,userId,customerId]);if(!result.rowCount)return false}await addProfessionalTimelineItem({workspaceId,userId,customerId,eventType:"next_action",title:"次回アクションをキャンセル",sourceRef:id});return true}
