...
export async function listCaptures(workspaceId: string, userId: string, customerId?: string): Promise<CaptureEntry[]> {
  ...
  if (mode === "d1") {
    ...
    const result = customerId
      ? await db.prepare("SELECT id,workspace_id,user_id,customer_id,kind,raw_text,created_at FROM velvet_professional_captures WHERE workspace_id=? AND user_id=? AND customer_id=? ORDER BY created_at DESC").bind(workspaceId, userId, customerId).all<CaptureRow>()
      : await db.prepare("SELECT id,workspace_id,user_id,customer_id,kind,raw_text,created_at FROM velvet_professional_captures WHERE workspace_id=? AND user_id=? ORDER BY created_at DESC").bind(workspaceId, userId).all<CaptureRow>();
    return result.results.map(mapRow);
  }
  if (mode !== "postgres") return entries.filter(entry => entry.workspaceId === workspaceId && entry.userId === userId && (!customerId || entry.customerId === customerId));
...
export async function getCapture(id: string, workspaceId: string, userId: string): Promise<CaptureEntry | undefined> {
  ...
  if (mode === "d1") {
    const db = await getD1Database();
    const row = await db?.prepare("SELECT id,workspace_id,user_id,customer_id,kind,raw_text,created_at FROM velvet_professional_captures WHERE id=? AND workspace_id=? AND user_id=? LIMIT 1").bind(id, workspaceId, userId).first<CaptureRow>();
    return row ? mapRow(row) : undefined;
  }
  if (mode !== "postgres") return entries.find(entry => entry.id === id && entry.workspaceId === workspaceId && entry.userId === userId);
...
export async function getCaptureSuggestions(workspaceId: string, userId: string, customerId?: string, limit = 18): Promise<CaptureSuggestion[]> {
  const [memory, dictionary] = await Promise.all([
  ...
  for (const item of dictionary) {
    ...
    const ageDays = Number.isFinite(lastUsed) ? Math.max(0, (now - lastUsed) / 86400000) : 365;
    const recencyScore = ageDays <= 1 ? 24 : ageDays <= 7 ? 18 : ageDays <= 30 ? 10 : ageDays <= 90 ? 4 : 0;
    add(item.displayValue, 30 + Math.min(35, item.useCount * 4) + recencyScore, "recent");
  }
  defaults.forEach((value, index) => add(value, 10 - index * .1, "default"));
...
export async function createCapture(input: { workspaceId: string; userId: string; customerId?: string; kind?: CaptureKind; value: string }): Promise<CaptureEntry | undefined> {
  ...
  const mode = getStorageMode();
  const entry: CaptureEntry = { id: mode === "d1" ? makeD1Id("capture") : makeId(), workspaceId: input.workspaceId, userId: input.userId, customerId: input.customerId, kind: input.kind ?? "free_text", value, createdAt: new Date().toISOString() };
  if (mode === "d1") {
    const db = await getD1Database();
    if (!db) throw new Error("D1_NOT_CONFIGURED");
    await db.prepare("INSERT INTO velvet_professional_captures (id,workspace_id,user_id,customer_id,kind,raw_text,created_at) VALUES (?,?,?,?,?,?,?)").bind(entry.id, entry.workspaceId, entry.userId, entry.customerId ?? null, entry.kind, entry.value, entry.createdAt).run();
  } else if (mode !== "postgres") entries.unshift(entry);
  else await dbQuery(`insert into velvet_professional_captures (id,workspace_id,user_id,customer_id,kind,raw_text,created_at) values ($1,$2,$3,$4,$5,$6,$7)`, [entry.id, entry.workspaceId, entry.userId, entry.customerId ?? null, entry.kind, entry.value, entry.createdAt]);
  ...
  if (entry.customerId && entry.kind === "conversation_note") {
    await addProfessionalTimelineItem({ workspaceId: entry.workspaceId, userId: entry.userId, customerId: entry.customerId, eventType: "conversation", title: "会話メモ", body: value, sourceRef: entry.id });
  } else if (entry.customerId && entry.kind !== "free_text") {
    const memory = await getCustomerMemory(entry.workspaceId, entry.userId, entry.customerId);
    const tags = Array.from(new Set([...(memory?.tags ?? []), ...value.split(/[、,\n]/).map(v => v.trim()).filter(Boolean)]));
    await upsertCustomerMemory(entry.workspaceId, entry.userId, entry.customerId, { tags });
  }
  return entry;
}
