...
function gatewayPath() {
  return process.env.AI_PLATFORM_CORE_GATEWAY_PATH?.trim() || "/v1/gateway/run";
}

function clientId() {
  return process.env.AI_PLATFORM_CORE_CLIENT_ID?.trim();
}
...
function safeJsonFromText(text: unknown): unknown {
...
async function runGateway(input: {
  capability: "velvet.capture.structure" | "velvet.search.parse_intent";
  permission: "velvet.capture.structure" | "velvet.search.parse_intent";
  ownerUserId: string;
  workspaceId?: string;
  goal: string;
  ...
  if (!endpoint || !client) return undefined;

  const response = await fetch(endpoint, {
    method: "POST",
    headers: { "Content-Type": "application/json", ...traceHeaders(input.trace) },
    cache: "no-store",
    body: JSON.stringify({
      auth: { clientId: client, permissions: [input.permission] },
      activity: {
        client,
        ...
        workflow: "velvet",
        goal: input.goal,
        context: { app: "Velvet", sourceApp: "velvet" },
        input: input.payload,
      },
      messages: [
        { role: "system", content: "Return JSON only. Do not add prose or markdown fences." },
  ...
  const body = await response.json() as {
    ok?: boolean;
    result?: { activityId?: string; output?: Record<string, unknown> };
  };
  if (!body.ok || !body.result) throw new Error("Invalid gateway response");
  const text = body.result.output?.text;
  return { activityId: body.result.activityId, parsed: safeJsonFromText(text) };
}
...
export async function parseSearchIntent(raw: string, ownerUserId: string, traceSeed?: Partial<TraceContext>): Promise<SearchIntentResult> {
  ...
  try {
    const result = await runGateway({
      ...
      permission: "velvet.search.parse_intent",
      ownerUserId,
      goal: "Parse a user-triggered Velvet search phrase into local search terms only.",
      payload: { query: raw },
      prompt: `Convert this Velvet search into JSON only: {"terms":["term1","term2"]}. Remove particles and generic words such as 人/客/探して. Do not infer facts. Query: ${JSON.stringify(raw)}`,
      trace,
    });
    const parsed = result?.parsed as { terms?: unknown } | undefined;
    const terms = Array.isArray(parsed?.terms) ? parsed.terms.map(String).map((v) => v.trim()).filter(Boolean) : [];
