...
import { removeRememberField, saveRememberField } from "./remember-actions";
...
export default async function RememberPage({ searchParams }: { searchParams: Promise<{ customerId?:string; savedLabel?:string; savedValue?:string; removedLabel?:string; error?:string; new?:string }> }) {
  const { customerId, savedLabel, savedValue, removedLabel, error, new: newCustomer } = await searchParams;
  ...
  const totalFilled=groups.reduce((sum,group)=>sum+countFilled(group.fields),0);
  ...
  return <main className="shell">
    ...
    {totalFilled>0&&<div className="card noticeCard"><div className="row"><div><div className="formHint">いま覚えていること</div><strong>{totalFilled}件</strong></div><span className="subtle">ここで終わってOK</span></div><div className="searchActions compactForm"><Link className="secondaryButton actionLink" href={`/people/${customerId}`}>この人を見る</Link><Link className="primaryButton actionLink" href={`/capture?customerId=${encodeURIComponent(customerId)}`}>今日の接客へ</Link></div></div>}
    ...
    {removedLabel&&<div className="card successCard"><div className="formHint">削除した内容</div><strong>{removedLabel}</strong></div>}
    {error&&<div className="formError">入力内容を確認してください。</div>}
    <div className="stack rememberGroups">{groups.map((group,index)=>{const filled=countFilled(group.fields);const recentlyChanged=Boolean((savedLabel&&group.fields.some(field=>field.label===savedLabel))||(removedLabel&&group.fields.some(field=>field.label===removedLabel)));const startOpen=isNewCustomer ? index<2 : index===0;return <details className="card rememberGroup" key={group.title} open={recentlyChanged || startOpen}><summary><div><strong>{group.title}</strong>{group.hint&&<div className="formHint">{group.hint}</div>}</div><span className="subtle">{filled>0?`${filled}件登録済み`:`開く`}</span></summary><div className="profileFields rememberGroupBody">{group.fields.map(field=>{const current=currentValue(field.label);return <div className="profileField" key={field.label}><div className="row"><strong>{field.label}</strong>{current&&<div className="row"><span className="subtle">登録済み：{current}</span><form action={removeRememberField.bind(null,customerId,field.label,isNewCustomer)}><button className="subtle" type="submit">削除</button></form></div>}</div>{field.examples.length>0&&<div className="chips">{field.examples.map(example=><form action={saveRememberField.bind(null,customerId,field.label,field.kind,example,isNewCustomer)} key={example}><button className={`chip chipButton${current===example?" selectedChip":""}`} type="submit">{example}</button></form>)}</div>}<form action={saveRememberField.bind(null,customerId,field.label,field.kind,undefined,isNewCustomer)} className="profileInputRow"><input className="searchBox" name="value" placeholder={current?"変更する場合は入力":"自由に入力"} autoComplete="off"/><button className="secondaryButton compactButton" type="submit">{current?"変更":"追加"}</button></form></div>})}</div></details>})}</div>
