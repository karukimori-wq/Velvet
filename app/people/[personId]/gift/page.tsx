  ...
  return (
    <main className="shell">
      <header className="header">
        ...
        <span className="subtle">Gift</span>
      </header>

      <section className="hero">
        <h1>{person.name}</h1>
        <p>もらった・あげたを最短で記録します。</p>
      </section>
      ...
      <form action={createGiftAction.bind(null, person.id, "received")} className="stack">
        <div className="chips">
          {commonItems.map((item) => <button className="chip chipButton" type="submit" name="item" value={item} key={`r-${item}`}>{item}</button>)}
        </div>
      ...
      <form action={createGiftAction.bind(null, person.id, "given")} className="stack">
        <div className="chips">
          {commonItems.map((item) => <button className="chip chipButton" type="submit" name="item" value={item} key={`g-${item}`}>{item}</button>)}
        </div>
        <input className="searchBox" name="item" placeholder="その他：品物を入力" autoComplete="off" />
      ...
      <details className="detailsCard">
        <summary>金額・理由・メモも追加</summary>
        <div className="stack detailsBody">
          <div className="formHint">詳細はクイック保存後、必要な場合だけ追記する設計です。MVPではまず最短記録を優先します。</div>
        </div>
      </details>
    </main>
  );
