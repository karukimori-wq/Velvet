...
export default async function CustomerDetailPage({ params, searchParams }: { params: Promise<{ customerId: string }>; searchParams: Promise<{ justEnded?: string }> }) {
  ...
  return <main className="shell">
    ...
    <div className="sectionTitle">クイック操作</div><div className="actions">{activeVisit ? <Link className="action actionLink activeAction" href={`/visits/${activeVisit.id}`}>来店中</Link> : <form action={startVisitAction}><input type="hidden" name="customerId" value={customerId} /><button className="action fullAction" type="submit">来店</button></form>}<Link className="action actionLink" href={`/capture?customerId=${customerId}`}>Capture</Link><Link className="action actionLink" href={`/people/${customerId}/message`}>文案</Link><Link className="action actionLink" href={`/people/${customerId}/gift`}>Gift</Link><Link className="action actionLink" href={`/relationships/new?customerId=${customerId}`}>関係</Link></div>
