...
export default async function CustomerDetailPage({ params, searchParams }: { params: Promise<{ customerId: string }>; searchParams: Promise<{ justEnded?: string; captureSaved?: string }> }) {
  ...
  return <main className="shell">
    ...
    <div className="sectionTitle">すぐ使う</div>
    <div className="searchActions">
      {activeVisit ? <Link className="primaryButton actionLink" href={`/visits/${activeVisit.id}`}>接客中を見る</Link> : <form action={startVisitAction}><input type="hidden" name="customerId" value={customerId} /><button className="primaryButton" type="submit">来店を記録</button></form>}
      <Link className="secondaryButton actionLink" href={`/capture?customerId=${customerId}`}>今日の接客を残す</Link>
    </div>
