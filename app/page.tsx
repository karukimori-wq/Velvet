...
export default async function HomePage() {
  ...
  const recent = memories.slice(0, 3).map((memory) => ({
    ...
    displayName: customerById.get(memory.customerId)?.displayName ?? memory.displayNameSnapshot ?? `Customer ${memory.customerId}`,
    tags: memory.tags,
  ...
  return <main className="shell">
    ...
    <Link href="/people" aria-label="Customerを検索"><div className="searchBox">名前・趣味・前回の話などで検索</div></Link>
    <div className="sectionTitle">最近の顧客メモ</div>
