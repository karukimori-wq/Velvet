...
export default async function HomePage(){
  ...
  return <main className="shell homeShell">
    ...
    <div className="homeGreeting"><span className="eyebrow">TODAY</span><strong>今日も、素敵な一日を。</strong><span>大切な人との時間を、もっと特別に。</span></div>
    <section className="metricGrid" aria-label="今日の概要">
      ...
      {access.soonAlertsAllowed?<Link className="metricCard" href="/people?sort=soon"><span className="metricIcon">⌛</span><span><span className="metricLabel">そろそろ</span><span className="metricValue">{soonAlerts.length}<small>名</small></span></span></Link>:<Link className="metricCard" href="/capture"><span className="metricIcon">✎</span><span><span className="metricLabel">記録する</span><span className="metricValue metricWord">今日の接客</span></span></Link>}
    </section>

    <section className={`card actionPanel${dueFollowups.length===0&&soonAlerts.length===0?" actionPanelQuiet":""}`}>
      <div className="row"><div><span className="eyebrow actionEyebrow">NEXT</span><div className="sectionTitle">本日のアクション</div></div><Link href="/people" className="actionMore">すべて見る ›</Link></div>
