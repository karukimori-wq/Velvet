export async function GET() {
  return Response.json({
    ...
    sourceOfTruth: {
      ...
      replyDraft: false,
      safetyCheck: false,
      communicationSendWorkflow: false,
      aiActivity: false,
      aiUsage: false,
      aiCapability: false
    },
    supportedApiOperations: ["VelvetVisit.Start","VelvetVisit.Complete","VelvetMemory.Get","VelvetMemory.Update","VelvetNote.Create","VelvetTimeline.List","VelvetNextAction.Create","VelvetHandoff.Start"],
    supportedEndpoints: ["POST /api/visits","PATCH /api/visits/{visitId}","GET /api/customers/{customerId}/memory","PATCH /api/customers/{customerId}/memory","POST /api/customers/{customerId}/notes","GET /api/customers/{customerId}/timeline","POST /api/customers/{customerId}/next-actions"],
    ...
    simpleMessageDraftIntegration: {
      targetApp: "sns-planner",
      owner: "sns-planner",
      operation: "MessageDraft.Generate",
      scope: "simple_business_initiated_contact_without_live_conversation_context_or_send",
      referenceOnly: true
    },
    communicationPlannerBoundary: {
      targetApp: "communication-planner",
      canonicalOwner: "communication-planner",
