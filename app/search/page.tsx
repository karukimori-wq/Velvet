...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; natural?: string }> }) {
  ...
  return (
    <main className="shell">
      ...
      {!query && <><div className="sectionTitle">例</div><div className="chips">{["ゴルフ", "ロレックス", "既婚", "響", "メガネ", "財布"].map((value) => <Link className="chip" href={`/search?q=${encodeURIComponent(value)}`} key={value}>{value}</Link>)}</div><div className="sectionTitle">文章でも検索</div><div className="chips">{["ゴルフ好きでロレックスの人", "響が好きな既婚の人", "財布をもらった人"].map((value) => <Link className="chip" href={`/search?q=${encodeURIComponent(value)}&natural=1`} key={value}>{value}</Link>)}</div></>}
      {query && <>
