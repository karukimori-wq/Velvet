...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; natural?: string }> }) {
  ...
  const [customers, memories, captures, gifts] = await Promise.all([
    listGrowthCustomers(workspaceId, userId), listCustomerMemories(workspaceId, userId), listCaptures(workspaceId, userId), listGifts(workspaceId, userId),
  ]);
  ...
  const captureResults = terms.length ? captures.filter((entry) => matchesAllTerms(entry.value, terms) && entry.customerId).slice(0, 10) : [];
  const giftResults = terms.length ? gifts.filter((gift) => matchesAllTerms([gift.item, gift.occasion, gift.note].filter(Boolean).join(" "), terms)).slice(0, 10) : [];
  ...
  return <main className="shell">
    ...
    <form action="/search" method="get" className="stack"><input className="searchBox" name="q" defaultValue={q} placeholder="例：ゴルフが好きでロレックスの人" autoComplete="off" /><div className="searchActions"><button className="secondaryButton" type="submit">そのまま検索</button><button className="primaryButton" type="submit" name="natural" value="1">文章で探す</button></div></form>
    {natural && query && <div className="card noticeCard"><div className="formHint">この条件で探しました</div><div className="timelineBody">{intent?.terms.length ? intent.terms.join(" ・ ") : query}</div></div>}
