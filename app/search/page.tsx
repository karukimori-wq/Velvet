...
import { listAllPersonProfiles } from "@/lib/person-profile-repository";
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; natural?: string }> }) {
  ...
  const [people, captures, gifts, contacts, profiles] = await Promise.all([
  ...
  const profileByPerson = new Map(profiles.map((profile) => [profile.personId, [
    ...
    profile.company,
    profile.area,
    profile.maritalStatus === "married" ? "既婚" : profile.maritalStatus === "unmarried" ? "未婚" : profile.maritalStatus === "unknown" ? "婚姻状況不明" : undefined,
  ].filter(Boolean) as string[]]));

  const personResults = terms.length ? people.filter((person) => {
    const haystack = [person.name, person.rank, ...person.personality, ...(profileByPerson.get(person.id) ?? []), ...(contactsByPerson.get(person.id) ?? []), ...person.timeline.flatMap((item) => [item.date, item.title, item.body])].filter(Boolean).join(" ");
  ...
  return (
    <main className="shell">
      ...
      <form action="/search" method="get" className="stack">
        <input className="searchBox" name="q" defaultValue={q} placeholder="名前・会社・エリア・連絡先・特徴・前回の話など" autoComplete="off" />
      ...
      {!query && <><div className="sectionTitle">例</div><div className="chips">{["ゴルフ", "ロレックス", "既婚", "会社名", "盛岡", "LINE ID"].map((value) => <Link className="chip" href={`/search?q=${encodeURIComponent(value)}`} key={value}>{value}</Link>)}</div><div className="sectionTitle">文章でも検索</div><div className="chips">{["ゴルフ好きでロレックスの人", "盛岡で会社経営の人", "財布をもらった人"].map((value) => <Link className="chip" href={`/search?q=${encodeURIComponent(value)}&natural=1`} key={value}>{value}</Link>)}</div></>}
      {query && <>
        <div className="sectionTitle">People · {personResults.length}件</div><div className="stack">{personResults.map((person) => <Link className="card personRow" href={`/people/${person.id}`} key={person.id}><div className="avatar">{person.name.slice(0, 1)}</div><div className="personMain"><div className="personName">{person.name}{person.rank ? ` · ${person.rank}` : ""}</div>{person.personality.length > 0 && <div className="personMeta">{person.personality.slice(0, 5).join(" · ")}</div>}</div><span>›</span></Link>)}</div>
        {giftResults.length > 0 && <><div className="sectionTitle">Gift</div><div className="stack">{giftResults.map((gift) => { const person = personMap.get(gift.personId); return <Link className="card" href={`/people/${gift.personId}`} key={gift.id}><div className="timelineTitle">{gift.direction === "received" ? "もらった" : "あげた"} · {gift.item}</div>{person && <div className="timelineBody">{person.name}</div>}</Link>; })}</div></>}
        {captureResults.length > 0 && <><div className="sectionTitle">Capture</div><div className="stack">{captureResults.map((entry) => { const person = entry.personId ? personMap.get(entry.personId) : undefined; const content = <div className="card"><div className="timelineTitle">{entry.value}</div>{person && <div className="timelineBody">{person.name}</div>}</div>; return entry.personId ? <Link href={`/people/${entry.personId}`} key={entry.id}>{content}</Link> : <div key={entry.id}>{content}</div>; })}</div></>}
