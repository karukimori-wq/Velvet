...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string; natural?: string }> }) {
  ...
  const customerResults = terms.length ? [...ids].flatMap((customerId) => {
    ...
    const proFields = advancedSearchAllowed
      ? [memory?.cautionNote, memory?.conversationSummary, memory?.lastInteractionSummary, memory?.nextTopicHint]
      : [];
    const haystack = [...basicFields, ...proFields].filter(Boolean).join(" ");
    return matchesAllTerms(haystack, terms)
      ? [{ customerId, displayName, tags: memory?.tags ?? [] }]
      : [];
  }) : [];
  ...
  const captureResults = advancedSearchAllowed && terms.length
    ? captures.filter((entry) => matchesAllTerms(entry.value, terms) && entry.customerId).slice(0, 10)
    : [];
  const giftResults = advancedSearchAllowed && terms.length
    ? gifts.filter((gift) => matchesAllTerms([gift.item, gift.occasion, gift.note].filter(Boolean).join(" "), terms)).slice(0, 10)
    : [];
  ...
  return <main className="shell">
    ...
    <form action="/search" method="get" className="stack">
      <input className="searchBox" name="q" defaultValue={q} placeholder={advancedSearchAllowed ? "例：ゴルフが好きでロレックスの人" : "例：ゴルフ ロレックス"} autoComplete="off" />
      <div className="searchActions">
        <button className="secondaryButton" type="submit">キーワード検索</button>
        {advancedSearchAllowed
          ? <button className="primaryButton" type="submit" name="natural" value="1">文章からキーワードを探す</button>
          : <Link className="primaryButton actionLink" href="/plans">Proで履歴まで探す</Link>}
      </div>
    </form>
    ...
    {useNatural && query && <div className="card noticeCard"><div className="formHint">文章からこのキーワードを取り出しました</div><div className="timelineBody">{localIntent?.terms.length ? localIntent.terms.join(" ・ ") : query}</div><div className="formHint">この検索はAIを使わず、Velvet内で処理します。</div></div>}
