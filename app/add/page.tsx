...
export default async function AddPage({ searchParams }: { searchParams: Promise<{ error?: string; name?: string }> }) {
  ...
  return <main className="shell">
    ...
    <div className="card">
      <strong>初めてのお客様</strong>
      <p className="subtle">まず名前だけ。分かったことはあとから足せます。</p>
      <form action={createCustomerFromVelvetAction} className="stack compactForm">
        <input className="searchBox" name="displayName" placeholder="名前・呼び名" defaultValue={name ?? ""} autoComplete="off" />
        {error === "name" && <div className="formError">名前か呼び名を入れてください。</div>}
        {error === "customer_create" && <div className="formError">お客様を登録できませんでした。少し時間をおいて再度お試しください。</div>}
        <button className="primaryButton" type="submit">登録して、この人を覚える</button>
      </form>
    </div>

    <div className="sectionTitle">登録済みのお客様</div>
    <div className="stack registerHub">
      <Link className="card registerChoice" href="/capture/profile"><div className="registerIcon">人</div><div><strong>この人について覚える</strong><p>外見・服装・持ち物・好み・仕事など</p></div><span>›</span></Link>
