  ...
  return (
    <main className="shell">
      ...
      <form action={createRelationshipAction} className="stack">
        <select className="selectBox" name="personAId" defaultValue={personId}><option value="">人物を選択</option>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select>
        <select className="selectBox" name="personBId" defaultValue=""><option value="">相手を選択</option>{people.filter((person) => person.id !== personId).map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select>
