    ...
    <main className="shell">
        ...
        <div className="stack">
          {entries.map((entry) => (
            <article className="card" key={entry.id}>
              <div className="row"><strong>{entry.title}</strong><span className="subtle">{kindLabel[entry.kind]}</span></div>
              {entry.startsAt && <div className="personMeta">{new Date(entry.startsAt).toLocaleString("ja-JP")}</div>}
              {entry.note && <div className="timelineBody">{entry.note}</div>}
            </article>
          ))}
        </div>
      )}
      ...
      <form action={createScheduleAction} className="stack">
        ...
        <div className="chips choiceRow">
          {(["visit", "shift", "birthday", "unavailable", "self_investment", "other"] as const).map((kind) => (
            <label className="choiceChip" key={kind}><input type="radio" name="kind" value={kind} defaultChecked={kind === "visit"} />{kindLabel[kind]}</label>
          ))}
        </div>
        <select className="selectBox" name="personId" defaultValue="">
          <option value="">人物なし</option>
