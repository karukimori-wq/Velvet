...
export default async function CapturePage({searchParams}:{searchParams:Promise<{customerId?:string;saved?:string;error?:string;fromVisit?:string}>}){
  ...
  if(!customerId){
    ...
    return <main className="shell captureShell"><AppHeader title="覚える"/><section className="hero capturePickerHero"><h1>誰との時間を覚えておきますか？</h1><p>接客の直後でも、覚えていることをそのまま数十秒でVelvetに残せます。</p></section><div className="capturePersonList">{customers.map(c=>{const name=c.displayName??"お客様";return <Link className="card personRow" href={`/capture?customerId=${encodeURIComponent(c.customerId)}`} key={c.customerId}><div className="avatar">{name.slice(0,1)}</div><div className="personMain"><div className="personName">{name}</div><div className="formHint">この人との出来事を覚える</div></div><span>›</span></Link>})}{customers.length===0&&<div className="card empty">登録済みのお客様がいません</div>}</div><BottomNav/></main>
  }
  ...
  return <main className="shell captureShell">
    ...
    <section className="captureFocusCard"><div className="captureFocusHeading"><strong>今日、覚えておくこと</strong><span>短いメモでも大丈夫です</span></div><form action={organizeCaptureAction.bind(null,customerId,fromVisit)} className="stack captureForm"><CaptureChatInput groups={rememberGroups} knownTags={memory?.tags??[]} phase={phase} voiceAllowed={access.voiceCaptureAllowed} placeholder={phase==="first"?"仕事、趣味、好みなど。分かったことをそのまま入力。":"今日話したこと、変わったこと、気づいたことを入力。"}/><button className="primaryButton" type="submit">整理して確認</button></form></section>
