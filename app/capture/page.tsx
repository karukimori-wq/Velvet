...
export default async function CapturePage({searchParams}:{searchParams:Promise<{customerId?:string;saved?:string;error?:string;fromVisit?:string}>}){
  ...
  const profileSummaryTitle=recallPhase.phase==="returning_after_gap"?"久しぶりなので人物情報を思い出す":"人物情報";
  ...
  const fieldCounts=new Map<string,number>(); for(const capture of captures){for(const label of capture.value.split(/[。\n]/).map(part=>part.split("：")[0]?.trim()).filter(Boolean) as string[])fieldCounts.set(label,(fieldCounts.get(label)??0)+1)} const frequentFieldLabels=[...fieldCounts.entries()].filter(([,count])=>count>=2).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([label])=>label);
  ...
  return <main className="shell captureShell">
    ...
    {phase==="repeat"&&(recall.items.length>0||recall.nextTopics.length>0)&&<details className="detailsCard captureRecall" open={presentation.profileOpen&&!fromVisit}><summary>{profileSummaryTitle}</summary><div className="stack detailsBody">{recall.items.map(item=><div key={`${item.label}-${item.value}`}><div className="formHint">{item.label}{item.freshness==="aging"?" · 少し前":""}</div><div className="timelineBody">{item.value}</div></div>)}{recall.nextTopics.length>0&&<div><div className="formHint">次に話す</div>{recall.nextTopics.map(topic=><div className="timelineBody" key={topic}>・{topic}</div>)}</div>}</div></details>}
