...
export default async function CapturePage({ searchParams }: { searchParams: Promise<{ customerId?: string; saved?: string; error?: string; fromVisit?: string }> }) {
  ...
  return <main className="shell">
    ...
    <section className="hero"><h1>{fromVisit && customerId ? `${displayName}の退店後メモ` : customerId ? `${displayName}を思い出す材料を残す` : "一言でも、すぐ残す。"}</h1><p>{fromVisit ? "覚えているうちに、スタンプと一言だけ。" : "まずタップ。足りない時だけ文字入力。"}</p></section>
    {saved && <div className="card successCard">保存しました。続けてタップできます。</div>}
    {error && <div className="formError">入力内容を確認してください。</div>}

    {fromVisit && customerId && <><div className="sectionTitle">会話メモ · タイムラインに残す</div><form action={conversationMemoAction.bind(null, customerId, fromVisit)} className="stack"><input className="searchBox" name="value" placeholder="例：来月大阪出張。娘の受験の話。" autoComplete="off" /><div className="searchActions"><button className="secondaryButton" type="submit" name="mode" value="save">メモだけ保存</button><button className="primaryButton" type="submit" name="mode" value="organize">保存して整理</button></div></form></>}
    ...
    <div className="sectionTitle">一言でまとめる</div><form action={organizeCaptureAction.bind(null, customerId)} className="stack"><input className="searchBox" name="value" placeholder="例：黒髪、ロレックス、来月大阪、犬飼った" autoComplete="off" /><button className="primaryButton" type="submit">保存して整理</button></form>
    ...
    <details className="detailsCard"><summary>自由メモだけ残す</summary><form action={captureAction.bind(null, customerId, "free_text")} className="stack detailsBody"><input className="searchBox" name="value" placeholder="あとで整理したい内容" autoComplete="off" /><button className="secondaryButton" type="submit">そのまま保存</button></form></details>
    <BottomNav />
  </main>;
}
