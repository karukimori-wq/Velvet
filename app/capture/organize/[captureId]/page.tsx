...
export default async function OrganizeCapturePage({ params, searchParams }: { params: Promise<{ captureId: string }>; searchParams: Promise<{ fromVisit?: string }> }) {
  ...
  return <main className="shell">
    ...
    <form action={confirmKnowledgeCandidatesAction.bind(null, capture.id, fromVisit)} className="stack compactForm">
      ...
      {nextTopics.length > 0 && <><div className="sectionTitle">次回話題</div><div className="chips reviewChips">{nextTopics.map((candidate,index)=><label className="choiceChip reviewChoice" key={`${candidate.value}-${index}`}><input type="radio" name="nextTopic" value={candidate.value} defaultChecked={index===0}/><span>{candidate.value}</span></label>)}</div></>}
