...
export async function confirmKnowledgeCandidatesAction(captureId: string, fromVisit: string | undefined, formData: FormData) {
  ...
  const [capture, access] = await Promise.all([
    getCapture(captureId, workspaceId, userId),
  ...
  const memoryTags = formData.getAll("memoryTag").map(String).map(v => v.trim()).filter(Boolean);
  const selected = formData.getAll("knowledge").map(String).map(v => v.trim()).filter(Boolean);
  const preferences = formData.getAll("preference").map(String).map(v => v.trim()).filter(Boolean);
  const nextTopics = formData.getAll("nextTopic").map(String).map(v => v.trim()).filter(Boolean);
  ...
  if (capture.customerId) {
    ...
    const structured = mergeMemoryTags(memory?.tags ?? [], memoryTags);
    const tags = Array.from(new Set([...structured, ...selected, ...preferences]));
    const existingPreferences = (memory?.preferenceNote ?? "").split("、").map(v => v.trim()).filter(Boolean);
  ...
  for (let i = 0; i < scheduleValues.length; i += 1) {
    const title = scheduleValues[i]?.trim();
    const raw = scheduleTimes[i]?.trim();
