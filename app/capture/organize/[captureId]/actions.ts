...
export async function confirmKnowledgeCandidatesAction(captureId: string, fromVisit: string | undefined, formData: FormData) {
  ...
  for (let i = 0; i < giftCount; i += 1) if (String(formData.get(`giftValue-${i}`) ?? "").trim().length > INPUT_LIMITS.giftItem) invalidSelection(capture.customerId, fromVisit);
  ...
  for (let i = 0; i < scheduleValues.length; i += 1) {
    const title = scheduleValues[i]; const raw = scheduleTimes[i]; if (!title || !raw) continue;
    const startsAt = new Date(raw); if (Number.isNaN(startsAt.getTime())) continue;
    await createScheduleEntry({ workspaceId, userId, customerId: capture.customerId, kind: "other", title, startsAt: startsAt.toISOString(), note: "会話から追加", idempotencyKey: `capture:${capture.id}:schedule:${i}` });
    scheduleCount += 1;
  }
