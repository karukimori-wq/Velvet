...
export function AddCustomerForm({ atLimit, customerCount, sourceAvailable, freePlan }: { atLimit: boolean; customerCount: number; sourceAvailable: boolean; freePlan: boolean }) {
  const [state, action, pending] = useActionState(createCustomerFromVelvetAction, initialState);
  const [displayName, setDisplayName] = useState("");
  const sourceBlocked = freePlan && !sourceAvailable;

  return <form action={action} className="stack compactForm">
    <label className="fieldLabel" htmlFor="displayName">名前・呼び名</label>
    <input id="displayName" className="searchBox" name="displayName" placeholder="例：あやさん" value={displayName} onChange={event => setDisplayName(event.target.value)} autoComplete="off" disabled={atLimit || sourceBlocked || pending} />
    ...
    {(sourceBlocked || state.error === "source_unavailable") && <div className="card noticeCard" role="alert"><strong>お客様情報を確認できません</strong><div className="formHint">Freeの30名上限を安全に確認できないため、今は新規登録を止めています。入力内容はこの画面に残っています。</div></div>}
    ...
    <button className="primaryButton" type="submit" disabled={atLimit || sourceBlocked || pending}>{pending ? "登録しています…" : "登録して、この人を覚える →"}</button>
